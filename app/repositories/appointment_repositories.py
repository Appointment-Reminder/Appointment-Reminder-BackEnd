...
class AppointmentRepository:
    ...
    def find_by_business(self, business_id: int):
        return self.db.exec(
            select(Appointment).where(Appointment.business_id == business_id)
        ).all()

    def get_appointments_by_photographer(self, photographer_id: int, status: Optional[str] = None):
    ...
    def get_appointments_by_id(self, appointment_id: int,user_id:Optional[int] = None, status : Optional[str] = None):
